  ...
  function Indexer() {
    ...
    _this.getIndex = function(docIndex) {
      ...
      if (docIndex === null) {
        ...
        // If a particular doc's index was given, create a subIndex with
        // the words that appear in the specified doc and return that.
    ...
    function readData(file) {
       ...
       * Utilise the relatively new `fetch` API to fetch and parse
       * the JSON file. Call `makeIndex` after a successful read.
       *
       * @param  {String} file path to file that's to be read.
       */

      if (RUNNING_ON_NODE) {
        _this.rawData = require('../books.json');
        makeIndex();
      } else {
        ...
        processData(completeData)
          .forEach(function(item) {
            if (!_this.index.hasOwnProperty(item)) {
              ...
              // Add the word's location to the index if the
              // location is not already associated with this word.
              // Otherwise ignore it.
    ...
    function processData(data) {
      ...
      if (Array.isArray(data)) {
        // If this data in an array, join it up to a long string
        // and process that.
        data = data.join(' ');
      }
